        ...
        if self._worker_task:
            try:
                ...
                if not loop.is_closed():
                    loop.create_task(self._worker_task)
            except:
                pass
        super().close()
...
class NextGenLoggingManager:
    ...
    def _setup_shutdown_hooks(self):
        """Configure les hooks d'arrêt propre"""
        import signal
        
        def shutdown_handler(*args):
            self._internal_logger.info("Arrêt du NextGeneration LoggingManager...")
            self.shutdown()
    ...
    def shutdown(self):
        ...
        try:
            ...
            print(f"Erreur arrêt: {e}", file=sys.stderr)
...
def get_agent_logger(agent_name: str, role: str, domain: str, agent_id: str = None) -> logging.Logger:
    ...
    return NextGenLoggingManager().get_agent_logger(agent_name, role, domain, agent_id)
