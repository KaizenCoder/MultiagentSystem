...
def appliquer_corrections():
    ...
    # Correction ligne 500: Ajout text() pour SQLAlchemy 2.x compatibility
    contenu_corrige = contenu_corrige.replace(
        r"""await conn.execute("VACUUM ANALYZE")""",
        r"""await conn.execute(text("VACUUM ANALYZE"))"""
    )
    corrections_appliquees += 1

    # Correction ligne 504: Ajout text() pour SQLAlchemy 2.x compatibility
    contenu_corrige = contenu_corrige.replace(
        r"""await conn.execute("ANALYZE")""",
        r"""await conn.execute(text("ANALYZE"))"""
    )
    corrections_appliquees += 1
