"""
🚀 REDIS CACHE PRODUCTION - PHASE 4 IA-2
...
class CacheConfig:
    ...
    # Cache TTL par stratégie (secondes)
...
class RedisProductionCache:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if await self._initialize_sentinel():
                logger.info("✅ Redis Sentinel initialisé avec succès")
                return True
                
            # Fallback : connexion directe cluster
            if await self._initialize_cluster():
                logger.info("✅ Redis Cluster initialisé avec succès")
                return True
                
            # Fallback : connexion single node (dev)
            if await self._initialize_single_node():
                logger.warning("⚠️ Redis single node initialisé (mode dev)")
                return True
                
            logger.error("❌ Échec initialisation Redis")
            return False
    ...
    def _hash_large_key(self, key: str) -> str:
        """Hash clés trop longues pour Redis"""
    ...
    async def set(self, strategy: CacheStrategy, key: str, value: Any,
        ...
        try:
            ...
            if result:
                ...
                logger.warning(f"⚠️ Cache SET failed: {cache_key[:50]}...")
                return False
    ...
    async def clear_pattern(self, pattern: str) -> int:
        ...
        try:
            if self.is_cluster_mode:
                # Pour cluster, itérer sur tous les nodes
                deleted = 0
...
if __name__ == "__main__":
    ...
    async def test_cache():
        print("🚀 REDIS PRODUCTION CACHE - TEST")
        print("=" * 50)
        ...
        if await cache.initialize():
            ...
            print(f"Opérations/sec: {benchmark['operations_per_second']}")
            ...
            print("❌ Échec initialisation cache")
