...
class TemplateManager:
    ...
    async def _preload_templates(self, template_names: List[str]) -> None:
        ...
        self.logger.info("Prchargement de %d templates...", len(template_names))
        
        tasks = [self._load_async(name) for name in template_names]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        success_count = sum(1 for r in results if not isinstance(r, Exception))
        self.logger.info("Prchargement termin : %d/%d succs", success_count, len(template_names))
    ...
    def reload_template(self, name: str) -> AgentTemplate:
        ...
        self.logger.info("Rechargement forc du template : %s", name)
        ...
        try:
            ...
            with self._lock:
                if template_name in self._cache:
                    agent_class = self._cache[template_name].to_class()
                else:
            ...
            self._metrics.errors[type(e).__name__] += 1
            self.logger.error("Erreur cration agent '%s': %s", template_name, e)
            raise
    ...
    def bulk_create_agents(
        ...
        for spec in specs:
            ...
            if not template_name:
                self.logger.warning("Spec ignore (pas de 'template') : %s", spec)
                continue
    ...
    def clear_cache(self) -> None:
        ...
        with self._lock:
            ...
            self._metrics.creation_times.clear()
            self.logger.info("Cache vid")
    ...
    def cleanup_old_entries(self, max_age_hours: int = 24) -> int:
        ...
        if removed:
            self.logger.info("Nettoyage cache : %d entres supprimes", removed)
    ...
    async def batch_operations(self):
        ...
        self.cache_ttl = float('inf')  # Cache infini pendant le batch
...
def get_template_manager(
    ...
    if force_new or _default_manager is None:
        _default_manager = TemplateManager(templates_dir, cache_size, ttl_seconds, enable_hot_reload, num_workers, preload_templates)
    
    return _default_manager


# Alias pour compatibilit
template_manager = get_template_manager(Path(), 128, 300, True, 4, None)
